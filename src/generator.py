...
def _format_context(context_docs):
    ...
    # 上下文预算：父块已按相关度降序，从高到低塞，塞满即停、末块按需截断。
    # 防止多个父块叠加超出免费 API 的 prompt token 上限。
    ...
    for i, doc in enumerate(context_docs, start=1):
        ...
        header = f"【来源{i} · {source} p.{page} · {mod}】\n"
        body = doc["content"]
        remain = budget - used - len(header)
        if remain <= 0:
            break
        if len(body) > remain:
            body = body[:remain].rstrip() + " …（已截断）"
