...
```bash
# 增量构建知识库（默认）：按 mtime+size 只解析新增/修改的文件，删除的文件清点，未变跳过
python ingest.py

# 全量重建：清库 + 忽略 manifest 重新解析所有文件
...
### 增量 ingest（`ingest.py` + `src/manifest.py` + `VectorStore.delete_by_source`）

`python ingest.py` 默认**增量**，不再无条件清库：

- `ingest_manifest.json` 记录每个文件的 `mtime+size`。每次跑用 `manifest.diff_data_dir` 分出新增/修改/删除/未变。
- **新增+修改** 的文件：先 `delete_by_source`（按 `metadata.source` 服务端 Filter 删该文件全部父+子块），再重解析入库。新增文件也删一次（干净库上是空操作），保证**无 manifest 的老库重跑也幂等、不产生重复点**。
- **删除** 的文件：清点。**未变**：完全跳过（零成本）。
- BM25 重建：`retriever.drop_sources(要清的来源)` 剔除留存子块中的旧来源，再拼上本次新子块，整体 `build_bm25_index`。BM25Okapi 不支持增量，但重建快。
- `python ingest.py --full` 走旧的全量路径（`reset_collection` + 忽略 manifest），用于彻底重建或修复 manifest/库不一致。
- 改解析逻辑后想让已入库文件重新解析：`--full`，或 `touch` 对应文件改 mtime。

### VectorStore 生命周期

- `VectorStore.__init__` 只确保 collection 存在，**不会**删数据。
- `reset_collection()` 清库重建，**只在 `--full` 模式调用**。增量模式用 `delete_by_source` 按文件精确删点，不要在增量路径里清全表。
- `Retriever` 接受外部 `vector_store` 参数；`ingest.py` 中要复用同一个实例，不要让 `Retriever()` 再造一个。
- `Retriever` 的 CrossEncoder reranker **懒加载**（`reranker` 属性首次访问才建）——ingest 不碰精排，省 ~1GB 内存降 OOM 风险。问答路径首次 rerank 时才加载。
